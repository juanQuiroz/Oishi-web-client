import React from "react";

const SalsaCounter = ({
  cantSalsas,
  salsa,
  totalSalsasSeleccionadas,
  setTotalSalsasSeleccioanda,
  salsasSeleccionadas,
  SetSalsasSeleccionadas,
}) => {
  console.log("🚀 ~ file: SalsaCounter.jsx ~ line 11 ~ salsa", salsa);

  const [cantSalsa, setCantSalsa] = React.useState(0);

  React.useEffect(() => {
    setTotalSalsasSeleccioanda(
      totalSalsasSeleccionadas.length == 0
        ? [{ ...salsa, id: salsa.id, cantSalsa: cantSalsa }]
        : totalSalsasSeleccionadas.some(tss => tss.id == salsa.id)
        ? totalSalsasSeleccionadas.map(tss =>
            tss.id == salsa.id
              ? { ...tss, id: salsa.id, cantSalsa: cantSalsa }
              : tss,
          )
        : [
            ...totalSalsasSeleccionadas,
            { ...salsa, id: salsa.id, cantSalsa: cantSalsa },
          ],
    );
    SetSalsasSeleccionadas(
      salsasSeleccionadas.length == 0 && cantSalsa > 0
        ? [{ ...salsa, cantSalsa: cantSalsa }]
        : salsasSeleccionadas.some(tss => tss.id == salsa.id)
        ? salsasSeleccionadas.map(tss =>
            tss.id == salsa.id ? { ...tss, cantSalsa: cantSalsa } : tss,
          )
        : [...salsasSeleccionadas, { ...salsa, cantSalsa: cantSalsa }],
    );
  }, [cantSalsa]);

  return (
    <div className="flex justify-between bg-white py-1 px-2 rounded-md mb-1">
      <p className="font-semibold"> {salsa.description}</p>{" "}
      <div className="flex justify-center ">
        <button
          type="button"
          onClick={() => {
            cantSalsa > 0 && setCantSalsa(cantSalsa - 1);
          }}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5 text-red-600"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zM7 9a1 1 0 000 2h6a1 1 0 100-2H7z"
              clipRule="evenodd"
            />
          </svg>
        </button>
        <input
          readOnly
          type="text"
          className="w-10 h-fit mx-1 rounded-md bg-oishiCeleste2 text-center text-black font-bold "
          value={cantSalsa}
        />
        <button
          type="button"
          onClick={() => {
            totalSalsasSeleccionadas.length > 0 &&
              totalSalsasSeleccionadas
                .map(tss => tss.cantSalsa)
                .reduce((prev, curr) => prev + curr, 0) < cantSalsas &&
              setCantSalsa(cantSalsa + 1);
          }}
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5 text-red-600"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
              fillRule="evenodd"
              d="M10 18a8 8 0 100-16 8 8 0 000 16zm1-11a1 1 0 10-2 0v2H7a1 1 0 100 2h2v2a1 1 0 102 0v-2h2a1 1 0 100-2h-2V7z"
              clipRule="evenodd"
            />
          </svg>
        </button>
      </div>
    </div>
  );
};

export default SalsaCounter;
